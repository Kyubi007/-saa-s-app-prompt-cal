...
"[project]/app/api/apply-drafts/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(request) {
    ...
    // Get all drafts
    const { data: drafts, error: fetchError } = await supabase.from("event_drafts").select("*").in("id", draftIds).eq("user_id", user.id);
    ...
    for (const draft of drafts){
        try {
            if (draft.type === "create") {
                ...
                const { data, error } = await supabase.from("events").update(updateData).eq("id", draft.target_event_id).eq("user_id", user.id).select().single();
                if (error) throw error;
                results.push({
                    draft_id: draft.id,
                    event: data,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__5ac3f8ef._.js.map